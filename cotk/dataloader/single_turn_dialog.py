...
class HGFSingleTurnDialog(BERTLanguageProcessingBase):
	r"""Base class for single-turn dialog datasets **with Hugging Face pytorch-transformer input**.
	This is an abstract class.
	...
	def get_batch(self, key, indexes):
		...
		Returns:
			(dict): A dict at least contains:
				...
				* post_allvocabs (:class:`numpy.array`): A 2-d padding array containing id of words in posts.
			  	...
			  	* post_bert (:class:`numpy.array`): A 2-d padding array containing BERT id of words in posts.
			  	  Size: `[batch_size, max(sent_length)]`
			  	* input_HGF (:class:`numpy.array`): A 2-d padding array containing HuggingFace id of words in posts concated resps.
			  	  Size: `[batch_size, max(post_sent_length + resp_sent_length - 1)]`
				* resp_length (:class:`numpy.array`): A 1-d array, the length of response in each batch.
			  	  Size: `[batch_size]`
				* resp (:class:`numpy.array`): A 2-d padding array containing id of words in responses.
			  	  Only provide valid vocabs. `unk_id` will be used if a word is not valid.
				...
				* resp_allvocabs (:class:`numpy.array`):
			  	...
			  	* resp_bert (:class:`numpy.array`):
			  	  A 2-d padding array containing BERT id of words in responses.
			  	  Size: `[batch_size, max(sent_length)]`
			  	* label_HGF (:class:`numpy.array`):
			  	  A 2-d padding array containing HuggingFace id of words in responses padded by -1.
			  	  Size: `[batch_size, max(post_sent_length + resp_sent_length - 1)]`
		'''
		if key not in self.key_name:
			raise ValueError("No set named %s." % key)
		...
		res_post = res["post"] = np.zeros((batch_size, np.max(res["post_length"])), dtype=int)
		res_resp = res["resp"] = np.zeros((batch_size, np.max(res["resp_length"])), dtype=int)
		res_post_bert = res["post_bert"] = np.full(
			(batch_size, np.max(res["post_length"])), self.bert_pad_id, dtype=int)
		res_resp_bert = res["resp_bert"] = np.full(
			(batch_size, np.max(res["resp_length"])), self.bert_pad_id, dtype=int)
		res_input_gpt = res["input_HGF"] = np.full(
			(batch_size, np.max(res["input_length"])), self.bert_pad_id, dtype=int)
		res_label_gpt = res["label_HGF"] = np.full(
			(batch_size, np.max(res["input_length"])), -1, dtype=int)
		for i, j in enumerate(indexes):
			post = self.data[key]['post'][j]
			resp = self.data[key]['resp'][j]
			res_post[i, :len(post)] = post
...
class HGFCleanWB(HGFSingleTurnDialog):
	'''A dataloader for CleanWB dataset.
